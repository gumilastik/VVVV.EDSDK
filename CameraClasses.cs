...
namespace EDSDKLib
{
	...
	public class SDKHandler : IDisposable
	{
		...
		/// </summary>
		///
		public uint Error = EDSDK.EDS_ERR_OK;  // Backing store
...
*/
		...
		public void CloseSession()
		{
			if (CameraSessionOpen)
			{
				...
				{
					StopLiveView();
					if(LVThread!= null) LVThread.Join(1000); // fix
				}
		...
		private uint Camera_SDKStateEvent(uint inEvent, uint inParameter, IntPtr inContext)
		{
			...
			switch (inEvent)
			{
				...
				//if(CameraSessionOpen) KeepAlive(); // Send keepalive next chance we get.
				break;
				...
				CameraSessionOpen = false;
				if (LVThread != null && LVThread.IsAlive) LVThread.Abort(); // fix
		...
		public void SetAF(EDSDK.EdsEvfAf state)
		{
			//start thread to not block everything
			//if (IsLiveViewOn) // fix
			SendSDKCommand(delegate
		...
		public void SetAFMode(uint af)
		{
			...
			SendSDKCommand(delegate
			{
				...
				lock (STAThread.ExecLock) {
					...
					EDSDK.EdsDataType proptype;
					//get size of property
					EDSDK.EdsGetPropertySize(MainCamera.Ref, EDSDK.PropID_Evf_AFMode, 0, out proptype, out propsize);
					//set given property
		...
		public void KeepAlive()
		{
			...
			{
				//send command to camera
				lock (STAThread.ExecLock) { Error = EDSDK.EdsSendCommand(MainCamera.Ref, EDSDK.CameraCommand_ExtendShutDownTimer, 0); };
				//lock (STAThread.ExecLock) { Error = EDSDK.EdsSendStatusCommand(MainCamera.Ref, EDSDK.CameraCommand_ExtendShutDownTimer, 0); };
			}, true);
		}
		...
		/// Takes a photo with the current camera settings
		/// </summary>
		public void TakePhotoShutterButton()
		{
			//start thread to not block everything
			SendSDKCommand(delegate
			{
